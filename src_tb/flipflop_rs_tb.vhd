...
-----Test du cas non specifie R S = "11", l'etat de sortie est indifferent
    report"Test du cas SR = 11, l'état de la sortie Q est indifferent";
    S_sti <= '1';
    R_sti <= '1';
    wait until Rising_Edge(Horloge_Int);
    Q_ref <= '-';
...
---------------------------------------------------------------------------
  ...
  begin
    ...
    --Verification asynchrone
      if Q_ref = '-' then
       report"Verif synchrone : état sortie Q pas vérifié";
      elsif Q_obs /= Q_ref then
    ...
    --Verification synchrone
      if Q_ref = '-' then
       report"Verif asynchrone : état sortie Q pas vérifié";
      elsif Q_obs /= Q_ref then
        Erreur_s <= '1', '0' after Pulse_c;
        Nbr_Err_v := Nbr_Err_v + 1;
